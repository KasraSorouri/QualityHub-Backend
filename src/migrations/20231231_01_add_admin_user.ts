...
module.exports = {
  up: async ({ context: queryInterface }: any) => {
    const userAdminExist = await queryInterface.sequelize.query("SELECT * FROM users WHERE username = 'admin'", {
    ...
    const [roleAdminExist] = await queryInterface.sequelize.query("SELECT * FROM roles WHERE role_name = 'Admin'", {
      type: QueryTypes.SELECT,
    });
    ...
    const [userAdmin]: User[] = await queryInterface.sequelize.query("SELECT * FROM users WHERE username = 'admin'", {
      type: QueryTypes.SELECT,
    });

    const [roleAdmin]: Role[] = await queryInterface.sequelize.query("SELECT * FROM roles WHERE role_name = 'Admin'", {
      type: QueryTypes.SELECT,
    });
