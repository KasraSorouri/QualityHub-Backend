...
const createClassCode = async (classCodeData: unknown): Promise<ClassCode> => {
  ...
  console.log(' ** materilas service * create ** classCode data ->', classCodeData);
  

  const newClassCodeData : ClassCodeData = await classCodeProcessor(classCodeData);

  console.log(' ** materilas service * create ** process data ->', newClassCodeData);
...
const updateClassCode = async (id: number, classCodeData: unknown): Promise<ClassCode>=> {  
  ...
  const newClassCodeData = await classCodeProcessor(classCodeData);
